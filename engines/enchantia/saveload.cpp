...
#define ENCHANTIA_SAVEGAME_VERSION 0
    ...
    EnchantiaEngine::kReadSaveHeaderError
    EnchantiaEngine::readSaveHeader(Common::SeekableReadStream *in, bool loadThumbnail, SaveHeader &header) {
    ...
    void EnchantiaEngine::savegame(const char *filename, const char *description) {
        ...
        out->writeByte(0);
        out->writeUint32LE(0);
        uint32 saveDate = ((curTime.tm_mday & 0xFF) << 24) | (((curTime.tm_mon + 1) & 0xFF) << 16) |
                          ((curTime.tm_year + 1900) & 0xFFFF);
        uint32 saveTime =
                ((curTime.tm_hour & 0xFF) << 16) | (((curTime.tm_min) & 0xFF) << 8) | ((curTime.tm_sec) & 0xFF);

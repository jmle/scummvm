...
// TODO Maybe move to Common since several engines use the RNC decompressor too (e.g. BASS, Toon...)
...
//calculate 16 bit crc of a block of memory
    ...
    int32 unpackRnc(const byte *input, byte *output) {
        RncDecoder rnc;
        int32 r;
        uint32 packLen = READ_BE_UINT32(input + 8);
        if (READ_BE_UINT32(input) == RNC1_SIGNATURE) {
            r = rnc.unpackM1(input, packLen, output);
        } else if (READ_BE_UINT32(input) == RNC2_SIGNATURE) {
            r = rnc.unpackM2(input, output);
        } else
            error("unpackRnc() Invalid compressed data");
        return r;
    }
